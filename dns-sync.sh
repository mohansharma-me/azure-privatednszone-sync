...
AUTH=$(http POST https://login.microsoftonline.com/${TENANT_ID}/oauth2/token content-type:"application/x-www-form-urlencoded" grant_type=client_credentials client_id="${CLIENT_ID}" client_secret="${CLIENT_SECRET}" resource="https://management.azure.com" --form --ignore-stdin)
ACCESS_TOKEN=$(echo "${AUTH}" | jq -r '.access_token')
printf " received of length : %s\n" "${#ACCESS_TOKEN}"
printf "Requesting DNS records..."
DNS_RESPONSE=$(http GET https://management.azure.com/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/${RESOURCE_GROUP}/providers/Microsoft.Network/privateDnsZones/${ZONE_NAME}/ALL api-version==2018-09-01 Authorization:"Bearer ${ACCESS_TOKEN}" --ignore-stdin)
printf " received of length : %s\n" "${#DNS_RESPONSE}"
